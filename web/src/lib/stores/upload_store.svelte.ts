...
export type Upload = {
    ...
    error?: string;
    duplicate?: { id: string, domain: string, name: string, author_name?: string };
    progress: number;
    ...
    while (uploadStore.enqueuedUploads.length > 0) {
        ...
        results.forEach((r, i) => {
            ...
            if (r instanceof APIError && r.message == "Duplicate trail") {
                u.status = "duplicate"
                u.duplicate = { id: r.detail.id, domain: r.detail.domain, name: r.detail.name, author_name: r.detail.author_name };
